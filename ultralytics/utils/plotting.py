...
def save_one_mask_crop(mask, img, file, padding=10):
    ...
    Args:
        ...
        file (Path | str): The path to save the cropped PNG image.
        padding (int): The number of pixels to add as padding around the crop.
    """
    # Ensure mask is a binary mask (0 or 255) and of type uint8
    mask = (mask > 0).astype(np.uint8) * 255

    # Find contours to get the bounding box of the object
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    if not contours:
        LOGGER.warning("No contours found in mask for saving mask crop.")
        return  # No object found in the mask
    ...
    # Create a 4-channel BGRA image (Blue, Green, Red, Alpha)
    bgra_img = cv2.cvtColor(cropped_img, cv2.COLOR_BGR2BGRA)

    # Set the alpha channel using the cropped mask
    bgra_img[:, :, 3] = cropped_mask

    # Save the image with transparency
    cv2.imwrite(str(file), bgra_img)
